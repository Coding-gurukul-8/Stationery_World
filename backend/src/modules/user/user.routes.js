// user.routes.js  —  Stationery World v4.0
...
const { requestOTP, verifyOTP, resetPassword } = require('./forgotPassword.controller');
...
// 🆕 User detail with total orders + spend (Section 7)
router.get('/:id/detail', authMiddleware, adminMiddleware, getUserDetail);

// Admin: update a user (isActive toggle, fields)
router.put('/:id', authMiddleware, adminMiddleware, async (req, res, next) => {
  try {
    ...
    if (req.body.role !== undefined) {
      return res.status(403).json({ success: false, message: 'Role cannot be changed here.' });
    }
